...
7.  **The Unconditional Pre-Execution Gatekeeper**: An AI agent operating under CSC is strictly prohibited from executing code-altering scripts, batch pipelines, or media generators without first reconciling the narrowest owning specification (`DELTALOG.md` / `CONTRACT.md`). The Contract is not a passive startup manual; it is an active, pre-execution runtime gatekeeper. If user intent has ambiguity, the agent must define the targeted scope in a `DELTA` proposal before invoking execution tools.
8.  **Interactive Cadence & Anti-Spin Invariant**: Background processes and asynchronous tasks must never decouple the agent from real-time communication with the Human-in-the-Loop. When an asynchronous task is launched, the agent MUST immediately conclude its tool sequence with clear, conversational human feedback and yield execution rather than spinning in autonomous status-checking loops.
9.  **Post-Edit Daemon & Template Refresh Invariant**: After ANY modification to HTML templates, view macros, routes, backend logic, or stylesheets, the agent MUST immediately execute the runtime restart/rebuild command (e.g., `./restart-gunicorn.sh`, `systemctl restart <daemon>`, or `npm run build`) before running verification checks or asking the maintainer to test in a browser. Asking a human to review a web surface while the active server is still executing stale in-memory templates is a strict boundary violation.
